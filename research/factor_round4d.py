#!/usr/bin/env -S uv run --no-sync
# -*- coding: utf-8 -*-
"""Round 4d 风险加固·风控层 —— 严格按 docs/factor_round4d_dual_breaker_plan.md v1.0 实施。

C 持仓 + 双熔断叠加：f = min(回撤熔断 f_4b, 均线择时 f_4c)。
与 C/4b/4c 四臂对照。判定看 R4d 的 L1~L4。
用法: uv run python research/factor_round4d.py
"""
from __future__ import annotations

import sys
from pathlib import Path

import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd

plt.rcParams["font.sans-serif"] = ["PingFang SC", "Heiti TC", "Arial Unicode MS"]
plt.rcParams["axes.unicode_minus"] = False

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from research.dividend_factor import load_all, month_last_days, metrics
from research.reversal_factor import ERAS, build_pool
from src.data_loader import load_index_daily

ROOT = Path(__file__).resolve().parent.parent
OUT = ROOT / "output"
R2 = ROOT / "data" / "round2"
COST = 15e-4
COST_SWEEP = [25e-4, 35e-4]
MIN_N = 50
MA_WIN = 200
BAND = 0.02
CUT_1, REST_1 = -0.20, -0.15
CUT_2, REST_2 = -0.30, -0.20


def _pct(vals: pd.Series) -> pd.Series:
    return vals.rank(pct=True)


def build_weight_matrix(ret: pd.DataFrame, sets: dict) -> pd.DataFrame:
    cols = ret.columns
    W = pd.DataFrame(0.0, index=ret.index, columns=cols)
    w = pd.Series(0.0, index=cols)
    for dt in ret.index:
        if dt in sets:
            S = sets[dt]
            w = pd.Series(0.0, index=cols)
            if S:
                w[list(S)] = 1.0 / len(S)
        W.loc[dt] = w
    return W


def scaled_nav(ret: pd.DataFrame, W: pd.DataFrame, f: pd.Series, cost: float) -> pd.Series:
    scaled = W.multiply(f, axis=0)
    turn = scaled.diff().abs().sum(axis=1).fillna(0.0)
    gross = (scaled.shift(1).fillna(0.0) * ret.fillna(0.0)).sum(axis=1)
    return (1 + gross - turn * cost).cumprod()


def drawdown_breaker(dd: pd.Series) -> pd.Series:
    f = pd.Series(1.0, index=dd.index)
    state = 1.0
    for t in dd.index:
        d = dd.loc[t]
        if state == 1.0 and d < CUT_1:
            state = 0.5
        elif state == 0.5:
            if d < CUT_2:
                state = 0.0
            elif d > REST_1:
                state = 1.0
        elif state == 0.0 and d > REST_2:
            state = 0.5
        f.loc[t] = state
    return f


def trend_breaker(idx_close: pd.Series) -> pd.Series:
    ma = idx_close.rolling(MA_WIN).mean()
    f = pd.Series(1.0, index=idx_close.index)
    state = 1.0
    for t in idx_close.index:
        c = idx_close.loc[t]
        if np.isnan(ma.loc[t]):
            f.loc[t] = state; continue
        if state == 1.0 and c < ma.loc[t] * (1 - BAND):
            state = 0.5
        elif state == 0.5 and c > ma.loc[t] * (1 + BAND):
            state = 1.0
        f.loc[t] = state
    return f


def max_dd_episode(nav: pd.Series) -> tuple[str, str, float]:
    cummax = nav.cummax()
    dd = nav / cummax - 1
    trough_i = dd.idxmin()
    peak_i = nav[:trough_i].idxmax()
    return str(peak_i.date()), str(trough_i.date()), float(dd.min())


def main() -> None:
    print("== Round 4d 双熔断叠加 (预注册 v1.0): C 持仓 + min(回撤, 趋势) ==")
    close, *_ = load_all()
    ret_qfq = close.pct_change()
    pool = build_pool(close, *load_all()[3:5])

    amt = pd.read_parquet(R2 / "daily_ext.parquet")
    amt["date"] = pd.to_datetime(amt["date"])
    amount = amt.pivot(index="date", columns="code", values="amount").sort_index()
    amount = amount.reindex(index=close.index, columns=close.columns).ffill()

    amihud = ((close.pct_change().abs() / amount) * 1e6).rolling(21, min_periods=15).mean()
    mom = close.shift(21) / close.shift(250) - 1.0

    idx = close.index
    sig_days = [t for t in month_last_days(idx) if idx.get_loc(t) + 1 < len(idx)]
    sets_C: dict = {}
    bench_sets: dict = {}
    for k, T in enumerate(sig_days):
        e = pool.loc[T]
        exec_day = idx[idx.get_loc(T) + 1]
        a = amihud.loc[T][e].dropna()
        m = mom.loc[T][e].dropna()
        common = a.index.intersection(m.index)
        if len(common) < MIN_N:
            continue
        if k + 1 < len(sig_days):
            bench_sets[exec_day] = set(common)
        score = (_pct(a.reindex(common)) + _pct(m.reindex(common))) / 2
        q = pd.qcut(score.rank(method="first"), 5, labels=False)
        sets_C[exec_day] = set(score.index[q == 4])

    W = build_weight_matrix(ret_qfq, sets_C)
    from research.reversal_factor import ew_nav
    nav_bench, _ = ew_nav(ret_qfq, bench_sets, COST)
    ann_bench = metrics(nav_bench)["年化"]

    navC = (1 + (W.shift(1).fillna(0.0) * ret_qfq.fillna(0.0)).sum(axis=1)
            - W.diff().abs().fillna(0.0).sum(axis=1) * COST).cumprod()
    dd_sig = navC / navC.cummax() - 1.0

    hs300 = load_index_daily("000300", start="20130601", refresh=False)["close"]
    hs300 = hs300.reindex(close.index).ffill()

    f4b = drawdown_breaker(dd_sig)
    f4c = trend_breaker(hs300)
    f4d = pd.concat([f4b, f4c], axis=1).min(axis=1)

    print(f"调仓期数 {len(bench_sets)}, 基准(公共池等权) 年化 {ann_bench:+.1%}")
    for k, fk in [("4b 回撤熔断", f4b), ("4c 均线择时", f4c), ("4d 双熔断", f4d)]:
        print(f"  {k}: 满仓 {(fk == 1.0).mean():.1%} 半仓 {(fk == 0.5).mean():.1%} "
              f"空仓 {(fk == 0.0).mean():.1%}")

    arms = {"C": navC,
            "R4d": scaled_nav(ret_qfq, W, f4d, COST)}
    rows = {}
    for k, nav in arms.items():
        pk, tr, mdd = max_dd_episode(nav)
        era_exc = {}
        for era, (s, e_) in ERAS.items():
            a, b = nav[s:e_], nav_bench[s:e_]
            era_exc[era] = metrics(a / a.dropna().iloc[0])["年化"] - \
                           metrics(b / b.dropna().iloc[0])["年化"]
        scaled = W.multiply(f4d if k == "R4d" else 1.0, axis=0)
        turn = float(scaled.diff().abs().sum(axis=1).sum() / 2 / (len(nav) / 244))
        nav35 = scaled_nav(ret_qfq, W, f4d if k == "R4d" else 1.0, COST_SWEEP[1])
        rows[k] = {
            "臂": k, "超额15bp": round(metrics(nav)["年化"] - ann_bench, 4),
            "超额35bp": round(metrics(nav35)["年化"] - ann_bench, 4),
            "年换手": round(turn, 1), "夏普": round(metrics(nav)["夏普"], 2),
            "最大回撤": round(mdd, 3), "回撤区间": f"{pk}~{tr}",
            **{f"超额{era}": round(v, 4) for era, v in era_exc.items()},
        }
        print(f"\n■ {k}: 超额15bp {rows[k]['超额15bp']:+.1%} | 35bp {rows[k]['超额35bp']:+.1%} "
              f"| 年换手 {turn:.1f} | 夏普 {rows[k]['夏普']:.2f} "
              f"| 最大回撤 {mdd:.1%} ({pk}~{tr})")
        print("   分段超额: " + "  ".join(f"{era}:{v:+.1%}" for era, v in era_exc.items()))

    ddC = navC / navC.cummax() - 1
    ddR = arms["R4d"] / arms["R4d"].cummax() - 1
    print("\n逐年最大回撤 (C / R4d):")
    for y in range(2015, 2027):
        sC, sR = ddC[f"{y}-01-01":f"{y}-12-31"], ddR[f"{y}-01-01":f"{y}-12-31"]
        if len(sC) and len(sR):
            print(f"  {y}: C {sC.min():.1%} | R4d {sR.min():.1%}")

    C, R = rows["C"], rows["R4d"]
    l1 = R["最大回撤"] >= -0.30
    l2 = R["超额15bp"] >= 0.04
    l3 = R["超额35bp"] > 0
    l4 = sum(R[f"超额{era}"] > 0 for era in ERAS) >= 2
    n_pass = sum([l1, l2, l3, l4])
    verdict = {4: "通过", 3: "部分通过"}.get(n_pass, "未通过")
    print(f"\n== 判定 (R4d vs C) ==")
    print(f"  L1 最大回撤≥-30%: C {C['最大回撤']:.1%} → R4d {R['最大回撤']:.1%} → {l1}")
    print(f"  L2 超额≥+4pp: R4d {R['超额15bp']:+.1%} → {l2}")
    print(f"  L3 35bp 超额>0: R4d {R['超额35bp']:+.1%} → {l3}")
    print(f"  L4 三段≥2 正: {sum(R[f'超额{era}'] > 0 for era in ERAS)}/3 → {l4}")
    print(f"  → R4d 判定: {verdict}({n_pass}/4)")

    pd.DataFrame(rows).T.to_csv(OUT / "factor_round4d_summary.csv")
    fig, ax = plt.subplots(figsize=(11.5, 6))
    ax.plot(navC, lw=1.2, label="C 满仓")
    ax.plot(arms["R4d"], lw=1.5, label="R4d C+双熔断")
    ax.plot(nav_bench, lw=1, alpha=0.6, color="gray", label="同池等权基准")
    ax2 = ax.twinx()
    ax2.fill_between(f4d.index, f4d.values, alpha=0.15, color="orange")
    ax2.set_ylabel("仓位 f")
    for y in ("2018-01-01", "2022-01-01"):
        ax.axvline(pd.Timestamp(y), color="gray", ls="--", lw=0.8)
    ax.set_yscale("log"); ax.legend(loc="upper left"); ax.grid(alpha=0.3)
    ax.set_title("Round 4d 双熔断叠加 (对数净值, 15bp; 橙色=仓位)")
    fig.tight_layout(); fig.savefig(OUT / "factor_round4d.png", dpi=130)
    print(f"\nCSV: output/factor_round4d_summary.csv | 图: output/factor_round4d.png")


if __name__ == "__main__":
    main()
