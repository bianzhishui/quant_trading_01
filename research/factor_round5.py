...
def main() -> None:
    ...
    print(f"调仓期数 {len(bench_sets)}, 中性化池月均 {np.mean(coverage):.0f} 只, "
          f"基准年化 {ann_bench:+.1%}")
    ...
    print(f"\n== 判定 (R5 vs C_base) ==")
    print(f"  N1 超额≥+4pp: C_base {Cb['超额15bp']:+.1%} → R5 {R['超额15bp']:+.1%} → {n1}")
    ...
    if n4 and not n1:
        print("  结论: 行业内 alpha 弱 → Amihud 流动性溢价主要是'跨行业'的，"
              "中性化后的诚实 alpha 很弱")
    elif n1 and n4:
    ...
    fig.tight_layout(); fig.savefig(OUT / "factor_round5.png", dpi=130)
