...
def main() -> None:
    ...
    for k in ["C", "R4a"]:
        print(f"  平均持股数 {k}: {np.mean(sizes[k]):.0f} 只 (中位 {int(np.median(sizes[k]))})")
    ...
    for k in ["C", "R4a"]:
        ...
        rows[k] = {
            "臂": k, "超额15bp": round(metrics(long_nav)["年化"] - ann_bench, 4),
    ...
    print(f"\n== 判定 (R4a vs C) ==")
    print(f"  H1 回撤改善≥3pp: C {C['最大回撤']:.1%} → R4a {R['最大回撤']:.1%} "
          f"(改善 {C['最大回撤']-R['最大回撤']:+.1%}) → {h1}")
    print(f"  H2 超额损失≤1.5pp: C {C['超额15bp']:+.1%} → R4a {R['超额15bp']:+.1%} "
    ...
    if h1:
        ...
        print("  结论: 回撤主要是市场/风格级，尾部剔除无效 → 下一步 Round 4b 转向回撤熔断/仓位管理")
    ...
    fig.tight_layout(); fig.savefig(OUT / "factor_round4.png", dpi=130)
