...
def trend_factor(idx_close: pd.Series) -> tuple[pd.Series, list]:
    ...
    for t in idx_close.index:
        ...
        if state == 1.0 and c < dn:
            state = 0.5; triggers.append((t, "1→0.5(跌破MA200)"))
        elif state == 0.5 and c > up:
            state = 1.0; triggers.append((t, "0.5→1(上穿MA200)"))
        f.loc[t] = state
    return f, triggers
...
def main() -> None:
    ...
    navR4c25 = scaled_nav(ret_qfq, W, f, COST_SWEEP[0])
    navR4c35 = scaled_nav(ret_qfq, W, f, COST_SWEEP[1])

    print(f"调仓期数 {len(bench_sets)}, 基准(公共池等权) 年化 {ann_bench:+.1%}")
    print(f"均线信号触发 {len(triggers)} 次 → {triggers[:3] + (['...'] if len(triggers) > 3 else [])}")
    ...
    fig.tight_layout(); fig.savefig(OUT / "factor_round4c.png", dpi=130)
