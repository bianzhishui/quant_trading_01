...
POOL = {"510300": "沪深300", "510500": "中证500", "159915": "创业板",
        "513100": "纳指", "518880": "黄金", "511010": "国债"}
BOND = "511010"
MOM_WIN, K = 20, 2
ERAS = {"2013-2017": ("2013-01-01", "2017-12-31"),
        "2018-2021": ("2018-01-01", "2021-12-31"),
        "2022-2026": ("2022-01-01", None)}          # 第三段=保留段
...
# ---------------- 信号与权重 ----------------
def momentum_weights(px: pd.DataFrame, variant: int,
                     mom_win: int = MOM_WIN, k: int = K) -> pd.DataFrame:
...
def backtest(px: pd.DataFrame, W: pd.DataFrame, cost_one_way: float) -> tuple[pd.Series, float]:
...
def metrics(nav: pd.Series) -> dict:
    ...
    if len(nav) < 50 or ret.std() == 0:
        return {"年化": np.nan, "波动": np.nan, "夏普": np.nan,
                "最大回撤": np.nan, "总收益": np.nan}
    years = len(nav) / 244
    return {"总收益": nav.iloc[-1] - 1,
...
def layer2(strat_m: pd.Series, ew_m: pd.Series) -> tuple[float, float]:
    ...
    arr = diff.to_numpy()
    obs = arr.mean()
    perm = np.array([rng.choice([1, -1], len(arr)).dot(arr) / len(arr)
...
def era_table(nav: pd.Series, ew_nav: pd.Series) -> list[dict]:
    ...
    for name, (s, e) in ERAS.items():
        ...
        rows.append({"年代": name, "年化": ma["年化"], "夏普": ma["夏普"],
                     "回撤": ma["最大回撤"], "等权年化": mb["年化"],
                     "跑赢等权": ma["年化"] > mb["年化"],
                     "夏普>0.3": ma["夏普"] > 0.3})
    return rows
...
def main() -> None:
    ...
    for label, v in variants.items():
        ...
        summary.append({"变体": label, "年化": m["年化"], "夏普": m["夏普"],
                        "回撤": m["最大回撤"], "等权年化": m_ew["年化"],
                        "②p值(置换)": pp, "③每段稳": stable,
                        "④10bp年超额": excess10, "④年换手": turn10,
                        "④可交易": tradable})

        print(f"\n■ {label}  (年换手 {turn5:.1f}@5bp / {turn10:.1f}@10bp)")
        print("  分年代稳定性(第③层):")
        for r in eras:
            print(f"   {r['年代']}: 年化 {r['年化']:+.1%} 夏普 {r['夏普']:.2f} "
    ...
    for drop in list(POOL):
        ...
        m = metrics(nav)
        print(f"  剔除 {POOL[drop]:<6}: 年化 {m['年化']:+.1%} 夏普 {m['夏普']:.2f} 回撤 {m['最大回撤']:.1%}")
    ...
    ax.text(pd.Timestamp(HOLDOUT_START), ax.get_ylim()[1] * 0.9,
            " 保留段起点(只揭榜一次)", fontsize=9, color="gray")
