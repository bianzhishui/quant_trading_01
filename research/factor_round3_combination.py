...
def _amihud(close, ext_amount): return (close.pct_change().abs() / ext_amount) * 1e6
def _mom(close, *_):            return close.shift(21) / close.shift(250) - 1.0
def _rev(close, *_):            return close / close.shift(21) - 1.0
...
def main() -> None:
    ...
    fv = {name: fn(close, amount).rolling(21, min_periods=15).mean() if name == "Amihud"
    ...
    for k, T in enumerate(sig_days):
        ...
        fwd = close.loc[sig_days[k + 1]] / close.loc[T] - 1.0 if k + 1 < len(sig_days) else None
        ...
        if len(common2) >= MIN_N:
            scoreC = (_pct(vals["Amihud"].reindex(common2), +1)
                      + _pct(vals["中期动量"].reindex(common2), +1)) / 2
        ...
        if len(common3) >= MIN_N:
            scoreD = (_pct(vals["Amihud"].reindex(common3), +1)
                      + _pct(vals["中期动量"].reindex(common3), +1)
                      + _pct(vals["反转"].reindex(common3), -1)) / 3
    ...
    names_arm = {"A": "Amihud 单因子", "B": "中期动量 单因子",
                 "C": "Amihud+动量", "D": "Amihud+动量+反转"}
    ...
    for k in ["A", "B", "C", "D"]:
        ...
        rows[k] = {
            "组合": names_arm[k], "年化15bp": round(metrics(long_nav)["年化"], 4),
            ...
            "年换手": round(turn, 1), "夏普": round(metrics(long_nav)["夏普"], 2),
        ...
        print(f"■ {names_arm[k]}: 超额15bp {rows[k]['超额15bp']:+.1%} "
              f"| 25bp {rows[k]['超额25bp']:+.1%} | 35bp {rows[k]['超额35bp']:+.1%} "
              f"| 换手 {turn:.1f} | 夏普 {rows[k]['夏普']:.2f} | 回撤 {rows[k]['回撤']:.1%} "
              f"| IC {ic_mean:+.4f}")
        print(f"   分段超额: " + "  ".join(f"{era}:{v:+.1%}" for era, v in era_exc.items()))
    ...
    gain = C["超额15bp"] - A_exc
    print(f"\n== 主判定 (C 臂) ==")
    print(f"  G1 组合不劣于最优单因子: C{A_exc:+.1%} vs A{A_exc:+.1%} → {g1} "
    ...
    d_verdict = ("反转在组合中被救活" if D_exc > A_exc and D_turn <= 6
                 else "反转仍是负资产/不改善")
    print(f"\n== D 臂独立结论 ==")
    print(f"  超额 {D_exc:+.1%} (A 基线 {A_exc:+.1%}), 换手 {D_turn:.1f} → {d_verdict}")

    # IC 序列相关（Amihud × 动量）
    ic_a = pd.Series(ics["A"]); ic_b = pd.Series(ics["B"])
    c_ab = ic_a.corr(ic_b)
    ...
    fig.tight_layout(); fig.savefig(OUT / "factor_round3.png", dpi=130)
    print(f"图: output/factor_round3.png")
