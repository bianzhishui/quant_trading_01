...
def main() -> None:
    ...
    corr_ic = ic_df.corr()          # 因子×因子（月度IC序列相关；bug修复：此前误用 ic_df.T.corr()）
    ...
    for name, direction in FACTORS.items():
        ...
        rows[name] = {
            ...
            "IC方向正确": sign_ok, "多头年化15bp": round(metrics(long_nav)["年化"], 4),
        ...
        print("  分组年化: " + "  ".join(
            ...
            + f"  基准:{ann_bench:+.1%}")
        print(f"  IC均值 {ic_mean:+.4f} (t={ic_t:+.2f}, 方向{'对' if sign_ok else '反'}) | "
    ...
    same_source = [(a, b) for i, a in enumerate(corr_cs.index) for b in corr_cs.index[i+1:]
    ...
    print(f"\n图已保存: {out_png}")
    print("CSV: factor_screen_round1_summary.csv / factor_corr_crosssec.csv / factor_corr_ic.csv")
