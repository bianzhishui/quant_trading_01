...
def save(path, buf):
    ...
    big = pd.concat([prev, pd.DataFrame(buf)], ignore_index=True) if prev is not None \
...
def main() -> None:
    ...
    print(f"公司行为: 共 {len(codes)}, 因子已有 {len(have_f)}, 分红已有 {len(have_d)}, "
          f"待抓 {len(todo)}", flush=True)
    lg = bs.login()
    ...
    while todo:
        ...
        for _ in range(3):
            try:
                rs = bs.query_adjust_factor(code=c, start_date="2012-01-01",
                                            end_date="2026-09-03")
                while rs.error_code == "0" and rs.next():
                    r = rs.get_row_data()
                    buf_f.append({"code": r[0], "date": r[1],
                                  "foreAdjustFactor": float(r[2])})
                    got = True
        ...
        if done % RELOGIN == 0:
            bs.logout(); lg = bs.login()
            print(f"  重连 [{len(todo)} 待抓]", flush=True)
        if streak >= EMPTY_STREAK:
            print(f"  疑似限流(连续{streak}只无数据), 休眠 {BACKOFF}s 后重连", flush=True)
            bs.logout(); time.sleep(BACKOFF); lg = bs.login()
            streak = 0
        if done % 200 == 0:
            save(OUTF, buf_f); save(OUTD, buf_d)
            print(f"  [{done}/{len(codes)-len(todo)}] 因子+{len(buf_f)} 分红+{len(buf_d)}"
                  f" 待抓 {len(todo)}", flush=True)
            buf_f, buf_d = [], []
        time.sleep(PAUSE)
    save(OUTF, buf_f); save(OUTD, buf_d)
