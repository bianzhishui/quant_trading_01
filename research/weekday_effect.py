...
def significance_tests(ret: pd.Series, n_perm: int = 10000) -> pd.DataFrame:
    ...
    for wd in range(5):
        ...
        rows.append({"星期": WEEKDAY_NAMES[wd],
                     "差异(bp)": obs_diff * 1e4,
                     "Welch_p": p_welch,
                     "置换_p": p_perm,
...
def stability_by_era(ret: pd.Series) -> pd.DataFrame:
    ...
    df["方向一致"] = [len({np.sign(v) for v in row}) == 1
                    for row in df[era_cols].to_numpy()]
    return df
...
def make_conclusion(sig: pd.DataFrame, stab: pd.DataFrame,
                    desc: pd.DataFrame, symbol: str) -> str:
    """综合三层 + 可交易性，生成一段结论。"""
    lines = [f"\n{'='*62}\n结论（{symbol}）\n{'='*62}"]
    sig_days = []
    for wd in WEEKDAY_NAMES:
        ...
        if p < ALPHA and consistent:
            lines.append(f"● {wd}: 效应存在且跨时期稳定 (p={p:.4f}, {edge_bp:+.1f}bp/天)")
            sig_days.append(wd)
        elif p < 0.05:
            lines.append(f"○ {wd}: p={p:.4f}<0.05 但未过 Bonferroni 校正或跨时期方向不一致 —— 按噪音处理")
        else:
            lines.append(f"× {wd}: 无显著差异 (p={p:.4f})")

    if not sig_days:
        lines.append("\n➤ 未发现任何同时满足[统计显著+跨时期稳定]的星期效应。")
        lines.append("  这与文献一致：早年记录的周内效应大多已衰减或本就是数据挖掘产物。")
    else:
        d = desc.loc[sig_days[0], "平均收益(bp)"] / 1e4
        annual_edge = d * 244
        annual_cost = COST_PER_ROUND_TRIP * 52
        lines.append(f"\n➤ 可交易性检验：即便效应为真，年化超额≈{annual_edge:.1%}，")
        lines.append(f"  而周频换仓的年成本≈{annual_cost:.1%} —— "
...
def main() -> None:
    ...
    # ---- 图表 ----
    fig, axes = plt.subplots(2, 1, figsize=(11, 8), gridspec_kw={"height_ratios": [1, 1]})
    ax = axes[0]
    ...
    colors = ["crimson" if sig.loc[w, "Bonferroni显著"] else "steelblue"
