...
COST_30 = 30e-4                 # ④ 压测档
ERAS = {"2018-2020": ("2018-01-01", "2020-12-31"),
...
def pick_sets(close: pd.DataFrame, prem: pd.DataFrame,
              factor: pd.DataFrame, sig_days: list[pd.Timestamp]) -> dict:
...
def rebalance_nav(close: pd.DataFrame, sets_exec: dict,
                  cost: float) -> tuple[pd.Series, float]:
...
def weekly_last_days(idx: pd.DatetimeIndex) -> pd.DatetimeIndex:
    iso = pd.Series(idx.isocalendar().week.astype(int) |
                    (idx.isocalendar().year.astype(int) * 1000), index=idx)
...
def main() -> None:
    ...
    sets_wk_main = signal_to_exec(pick_sets(close, prem, dbl, list(wk_days)), close.index)
    sets_mo_main = signal_to_exec(pick_sets(close, prem, dbl, list(mo_days)), close.index)
    sets_wk_prem = signal_to_exec(pick_sets(close, prem, prem, list(wk_days)), close.index)
    sets_wk_px = signal_to_exec(pick_sets(close, prem, close, list(wk_days)), close.index)

    sizes = [len(s) for s in sets_wk_main.values() if s]
    print(f"周频入选数: 中位 {int(np.median(sizes))}, 范围 {min(sizes)}~{max(sizes)} "
          f"(2018初可选池较小属预期)\n")
    ...
    print("== 汇总 (5bp 基础成本) ==")
    rows = []
    for label, m, tn in [("主组合 双低20·周频", mMain, turn_main),
                         ("月频变体", mMo, turn_mo),
                         ("低溢价前20(归因)", mP, np.nan),
    ...
    for name, (s, e) in ERAS.items():
        ...
        stable &= ok
        print(f"  {name}: 主 {ma['年化']:+.1%} 夏普 {ma['夏普']:.2f} | "
    ...
    # ---- 归因读数 ----
    print(f"\n== 归因读数 ==")
    print(f"周频 vs 月频: {mMain['年化']:+.1%} vs {mMo['年化']:+.1%} "
          f"→ 周频边际价值 {mMain['年化'] - mMo['年化']:+.1%}/年 (换手 {turn_main:.0f} vs {turn_mo:.0f})")
    print(f"低溢价组 {mP['年化']:+.1%} | 低价格组 {mPx['年化']:+.1%} | 主组合 {mMain['年化']:+.1%}")
    if mP["年化"] > mPx["年化"]:
    ...
    out = Path(__file__).resolve().parent.parent / "output" / "convertible_double_low.png"
