...
def breaker_factor(dd: pd.Series) -> pd.Series:
    ...
    for t in dd.index:
        ...
        if state == 1.0 and d < CUT_1:
            state = 0.5; triggers.append((t, "1→0.5"))
        elif state == 0.5:
            if d < CUT_2:
                state = 0.0; triggers.append((t, "0.5→0"))
            elif d > REST_1:
                state = 1.0; triggers.append((t, "0.5→1"))
        elif state == 0.0 and d > REST_2:
            state = 0.5; triggers.append((t, "0→0.5"))
        f.loc[t] = state
    return f, triggers
...
def main() -> None:
    ...
    # 25/35bp 压测：换手成本变，但 f/触发不变（触发只看 C 满仓净值）
    navR4b25 = scaled_nav(ret_qfq, W, f, COST_SWEEP[0])
    navR4b35 = scaled_nav(ret_qfq, W, f, COST_SWEEP[1])

    print(f"调仓期数 {len(bench_sets)}, 基准(公共池等权) 年化 {ann_bench:+.1%}")
    print(f"熔断触发次数: {len(triggers)} → {triggers if len(triggers) <= 8 else triggers[:4] + ['...']}")
    print(f"熔断期间(净值日)占比: {(f < 1.0).mean():.1%} (满仓 {(f == 1.0).mean():.1%}, "
    ...
    fig.tight_layout(); fig.savefig(OUT / "factor_round4b.png", dpi=130)
