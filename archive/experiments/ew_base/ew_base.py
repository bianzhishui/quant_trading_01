...
def main() -> None:
    ...
    m = metrics(ew_daily)
    print(
        f"■ EW-日频(零成本理论上限): 年化 {m['年化']:+.1%} 夏普 {m['夏普']:.2f} "
        f"回撤 {m['最大回撤']:.1%}"
    )
    ...
    for code, name in [
        ...
        try:
            ...
            m = metrics(s)
            print(
                f"■ {name}(真实指数): 年化 {m['年化']:+.1%} 夏普 {m['夏普']:.2f} "
                f"回撤 {m['最大回撤']:.1%}"
            )
    ...
    print("\n== 关键读数 ==")
    print(
        f"EW-年频 vs 真实中证500: {mA['年化']:+.1%} vs {m5['年化']:+.1%} "
    ...
    for name, (s, e) in ERAS.items():
        ...
        total += 1
        print(
            f"  {name}: EW-年频 {ma['年化']:+.1%} vs 中证500 {mb['年化']:+.1%} "
            f"→ {'跑赢' if win else '跑输'}"
        )
    ...
    print(
        "⚠ 判定解释: 本实验池=当前成分, 差值中含幸存者偏差, 不可全额外推;"
