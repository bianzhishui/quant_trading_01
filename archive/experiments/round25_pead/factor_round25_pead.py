...
# -*- coding: utf-8 -*-
"""Round 25 PEAD / 业绩预告后漂移因子筛选（预注册）。

因子 = 月末 T 取该股最近一次已公告(公告日≤T)的净利润业绩预告同比幅度(%), 方向+1。
数据: data/round2/earnings_forecast.parquet (akshare 抓取, 158k行 2014-2026)。
框架沿用 Round 1/2 四层判定 + 与 Amihud/动量 相关矩阵。

用法: python research/factor_round25_pead.py
"""
...
MIN_N = 50
FORECAST_FILE = Path("data/round2/earnings_forecast.parquet")
NET_PROFIT_IND = {"净利润", "归属于上市公司股东的净利润"}
MATRIX_MEMBERS = ["PEAD", "Amihud", "中期动量"]


def build_pead_factor(close: pd.DataFrame) -> pd.DataFrame:
    """PEAD 宽表(date×code): 最近已公告净利润预告同比幅度(%, ffill)。"""
    ann = pd.read_parquet(FORECAST_FILE)
    ann = ann[ann["预测指标"].isin(NET_PROFIT_IND)].copy()
    ann["公告日期"] = pd.to_datetime(ann["公告日期"])
    ann["code"] = ann["股票代码"].astype(str).str.zfill(6)
    ann["code_full"] = np.where(
        ann["code"].str.startswith(("60", "68")),
        "sh." + ann["code"],
        "sz." + ann["code"],
    )
    ann = ann[["code_full", "公告日期", "业绩变动幅度"]].dropna(subset=["业绩变动幅度"])
    fac = pd.DataFrame(index=close.index, columns=close.columns, dtype=float)
    for code in close.columns:
        sub = ann[ann["code_full"] == code].sort_values("公告日期")
        if sub.empty:
            continue
        s = sub.set_index("公告日期")["业绩变动幅度"]
...
def main() -> None:
    print("== Round 25 PEAD / 业绩预告后漂移 (预注册 v1.0) ==")
    ...
    c1 = ic_mean > 0 and abs(ic_mean) >= 0.03 and abs(ic_t) >= 2
